...
pub async fn btx(bot: &mwbot::Bot) -> Result<(), Box<dyn std::error::Error>> {
    let xml = include_str!("../../xmls/xmlname.xml");
    ...
    for (i, page) in pages.iter().enumerate() {
        ...
        let result: Result<Value, _> = bot
            ...
            .post_with_token("csrf", &[
                ...
                ("title", &title),
                ("text", &text),
                ("summary", format!("Import: {} ({}): {}", contributor, timestamp, summary).as_str()),
                ("contentmodel", &model),
                ("bot", "1"),
                ("createonly", "1"),
                ("watchlist", "nochange"),
